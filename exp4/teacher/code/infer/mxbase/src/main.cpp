...
int main(int argc, char* argv[]) {
    ...
    initParam.modelPath = "../data/model/vgg17.om";
    auto vgg17 = std::make_shared<Vgg17Classify>();
    APP_ERROR ret = vgg17->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "Vgg17Classify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto & entry : fs::directory_iterator(imgDir)) {
        ...
        ret = vgg17->Process(entry.path());
        if (ret != APP_ERR_OK) {
            LogError << "Vgg17Classify process failed, ret=" << ret << ".";
            vgg17->DeInit();
            return ret;
        }
    }
