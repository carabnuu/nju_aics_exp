...
APP_ERROR Vgg17Classify::Init(const InitParam &initParam) {
...
APP_ERROR Vgg17Classify::DeInit() {
    dvppWrapper_->DeInit();
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR Vgg17Classify::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR Vgg17Classify::Resize(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
...
APP_ERROR Vgg17Classify::Crop(const cv::Mat &srcMat, cv::Mat &dstMat) {
    static cv::Rect rectOfImg(16, 16, 224, 224);
    dstMat = srcMat(rectOfImg).clone();
    return APP_ERR_OK;
}

APP_ERROR Vgg17Classify::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR Vgg17Classify::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Vgg17Classify::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Vgg17Classify::Process(const std::string &imgPath) {
    cv::Mat imageMat;
...
APP_ERROR Vgg17Classify::SaveResult(const std::string &imgPath, const std::vector<std::vector<MxBase::ClassInfo>> \
                                              &BatchClsInfos) {
